...
class StateStarShipsWidget extends State<StarShipsWidget> {
  ...
  void initState() {
    super.initState();
    futurePlanets =
        fetchCharacterListRequestModel('https://swapi.dev/api/starships/');
  }
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                for (var element in snapshot.data!.results) {
                  listCharacterModel.add(StarshipsModel.fromJson(element));
                }
                return ListView.builder(
                  itemBuilder: (context, index) => Text(
                      '${listCharacterModel[index].name}\n        ${listCharacterModel[index].cargoCapacity}'),
